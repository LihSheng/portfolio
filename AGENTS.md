...
**Contact.** `components/ContactForm.tsx` posts to `app/api/contact/route.ts`, a thin handler that delegates to `lib/contact/submit-contact-message.ts`. That module validates with `lib/contact/schema.ts` (zod; re-exported as `lib/validation.ts` for backward compatibility), checks the honeypot `website` field via `lib/contact/spam-policy.ts`, rate-limits by IP via `lib/contact/rate-limit.ts` (in-memory, 5 requests / 15 min by default — resets on redeploy, not for production scale), and delivers via an adapter from `lib/contact/delivery.ts` / `lib/contact/adapters/*` chosen by `EMAIL_PROVIDER`: `console` | `nodemailer` | `formspree`. `lib/email.ts` is unused legacy code kept for reference — don't wire new code to it. Unit tests: `tests/contact/submit-contact-message.test.ts` (run with `node --experimental-strip-types --test tests/contact/*.test.ts`). Env vars: `NEXT_PUBLIC_CONTACT_EMAIL`, and either `SMTP_HOST`/`SMTP_PORT`/`SMTP_USER`/`SMTP_PASS` or `FORMSPREE_ENDPOINT`. See `.env.example`.
...
- The CV is `public/Ng-Lih-Sheng-Resume.pdf`, linked from the home, about, and footer components. Don't reintroduce `/cv.pdf`.
- The profile picture is `public/images/profile.png`, resolved via `siteConfig.author.profilePicture` (`NEXT_PUBLIC_PROFILE_PICTURE_URL` env var, falling back to `/images/profile.png`) and rendered by `components/ProfilePicture.tsx` on the About page.
